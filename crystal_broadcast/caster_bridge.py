# Caster bridge: forward battle commentary beats from the live player to the
# caster (crystal_broadcast/caster.py), which generates PRISM's and FRACTURE's lines.
#
# HISTORY, because the wire format looks arbitrary otherwise: this shape was
# reverse-engineered from AIRI 0.11.0's embedded @proj-airi/server-runtime,
# back when the plan was for AIRI to be the voice and face. caster.py was
# written to speak the same protocol so it could stand in for AIRI, and then
# it simply replaced it — AIRI is an inspiration for this project, not a
# dependency, and nothing here has talked to the app since the duo shipped.
# The format stayed because it works and both ends already spoke it. It is
# ours now, so it can be simplified whenever this layer is touched; the
# handshake in particular is vestigial (the caster accepts any local token).
#
# Protocol:
#   endpoint  ws://127.0.0.1:8131/ws          (root path 404s every frame)
#   ->  {"type": "module:authenticate", "data": {"token": <any local string>}}
#   <-  {"json": {"type": "module:authenticated", "data": {"authenticated": true}}}
#       (server frames arrive superjson-wrapped; plain {type,data} is accepted
#        inbound — unwrap with msg.get("json", msg))
#   ->  {"type": "input:text", "data": {"text": ...}}   the beat
#   <-  {"json": {"type": "output:gen-ai:chat:complete", "data": {...}}} per line
#
# There is no app-level heartbeat event: any frame refreshes peer health
# (ttl 60s), and the websockets library's automatic protocol pings cover
# quiet stretches.
#
# Manual test (the caster must be running):
#   .venv/bin/python crystal_broadcast/caster_bridge.py "Mic check from the bridge."
#
# Live commentary tap (for recording sessions — prints each battle beat and
# the spoken reply as they happen):
#   .venv/bin/python crystal_broadcast/caster_bridge.py --watch

from __future__ import annotations

import asyncio
import html
import json
import re
import sys

from pathlib import Path

import websockets

# The caster (crystal_broadcast/caster.py). This used to default to AIRI on :6121 —
# AIRI is an inspiration for this project, not a dependency, and nothing in
# the broadcast path has talked to it since the duo shipped.
DEFAULT_URL = "ws://127.0.0.1:8131/ws"
# The handshake is vestigial: the caster accepts any local token. It used to
# be read fresh from AIRI's ~/.config/ai.moeru.airi/server-channel-config.json
# (the app regenerated it), which meant the bridge could not connect unless
# the AIRI desktop app was installed. Nothing verifies it now.
LOCAL_TOKEN = "local"


def _load_token() -> str:
    return LOCAL_TOKEN


_CODE_FENCE = re.compile(r"```.*?```", re.S)
# a leaked feed echo: the model repeats the beat it was given. Everything
# from here on is regurgitation/reasoning, never broadcast copy.
_BEAT_ECHO = re.compile(r"[_*`\s]*\[\s*(?:BATTLE|MATCH|RESULT|bridge-test)\b",
                        re.I)
# thinking-tag scaffolding: a reply that opens (or continues) into an
# XML-ish thought tag is chain-of-thought, never broadcast copy. Replies
# sometimes arrive HTML-escaped ("&lt;thought"), so _sanitize unescapes
# entities before this runs.
_TAG_SCAFFOLD = re.compile(
    r"<\s*/?\s*(?:thought|think|thinking|reasoning|reason|scratchpad|"
    r"internal|analysis)\b", re.I)
_NOTE_PAREN = re.compile(r"[_*\s]*\((?:note|aside|correction|edit)\b[^)]*\)[_*]*",
                         re.I)
_META_SENTENCE = re.compile(
    r"(?:^|(?<=[.!?]))\s*[^.!?]*\b(?:let me re-?verify|re-?verify|"
    r"the previous (?:t\d+|turn|line)|i should (?:re-?)?check|as an ai|"
    r"i cannot|i can't help|battle log|ongoing match|as the commentator|"
    r"this (?:is|provides) the (?:battle )?log|my (?:role|task) is)\b"
    r"[^.!?]*[.!?]", re.I)
_SENTENCE = re.compile(r"[^.!?]*[.!?]+")
_MAX_SENTENCES = 3
# thinking-mode tells: once any of these appears the rest is the model's
# scaffolding (self-talk, re-drafts, stage directions), never broadcast copy
_THINKING_TELL = re.compile(
    r"\(\s*(?:done|final|end|executing|ready|proceeding|confidence|"
    r"self-correction|outputting|the end|no more|start outputting|bye)"
    r"|\bActually, I\b|\bFinal (?:Response|Final|answer|decision|String)\b"
    r"|\binternal (?:thought|monologue|dialogue)\b|\bI will just output\b"
    r"|\bI (?:must not|should not) (?:use|say)\b|\bLet me re-?"
    r"|\bLet's go\b|\[END\]|\[Final|\bEnd of thought\b|\bStop thinking\b",
    re.I)


def _sanitize(text: str) -> str:
    """Reduce a raw model reply to clean, spoken broadcast copy.

    Two failure modes to defend against: (1) the character echoes the
    '[BATTLE T8] ...' beat it was fed and rambles about it; (2) with
    thinking mode on, Gemma dumps its whole chain-of-thought ('Final
    Final... Executing... End trace'). A legitimate Prism reply is a single
    line of 1-2 sentences, so: keep only the first paragraph, cut at the
    first feed echo or thinking-tell, drop notes/meta/markdown, and hard-cap
    the sentence count. Nothing multi-paragraph or scaffold-shaped survives."""
    text = html.unescape(text)              # "&lt;thought" -> "<thought"
    text = _CODE_FENCE.sub("", text)
    # a real reply is one line of prose. Wrap-up replies often open with an
    # echoed "[RESULT] ..." line; skip leading blank/bracket-led lines and
    # speak the first real one (an echo line is never worth salvaging)
    lines = text.split("\n")
    idx = 0
    while idx < len(lines) and (not lines[idx].strip()
                                or lines[idx].lstrip().startswith("[")):
        idx += 1
    text = lines[idx] if idx < len(lines) else ""
    m = _BEAT_ECHO.search(text)
    if m:
        text = text[:m.start()]
    m = _TAG_SCAFFOLD.search(text)
    if m:
        text = text[:m.start()]
    m = _THINKING_TELL.search(text)
    if m:
        text = text[:m.start()]
    text = _NOTE_PAREN.sub("", text)
    text = _META_SENTENCE.sub("", text)
    text = re.sub(r"[_*`#>]{1,}", "", text)   # markdown emphasis/heading/quote
    text = re.sub(r"\s+", " ", text).strip()
    if text.startswith("["):                  # timestamp/beat fragment, not prose
        return ""
    sentences = _SENTENCE.findall(text)
    if len(sentences) > _MAX_SENTENCES:
        text = "".join(sentences[:_MAX_SENTENCES]).strip()
    elif sentences and not text.rstrip().endswith(tuple(".!?…")):
        # max_tokens cut mid-sentence ("... DARKRAI IN,"): drop the
        # dangling fragment, keep the complete sentences before it
        text = "".join(sentences).strip()
    return text


def _unwrap(raw: str | bytes) -> dict:
    """Server frames are superjson envelopes: {"json": {...}, "meta": {...}}."""
    msg = json.loads(raw)
    return msg.get("json", msg) if isinstance(msg, dict) else {}


class CasterBridge:
    """Fire-and-forget text events to the caster over its WebSocket.

    send() is thread-safe and never blocks or raises: events are queued on
    the asyncio loop captured at start(), a background task owns the
    connection, and a dead/absent caster only costs a retry loop — the game
    must never stall or crash over commentary.
    """

    def __init__(self, url: str = DEFAULT_URL, name: str = "crystal-battle"):
        self._url = url
        self._name = name
        self._queue: asyncio.Queue[tuple] = asyncio.Queue(maxsize=32)
        self._loop: asyncio.AbstractEventLoop | None = None
        self._task: asyncio.Task | None = None
        self._connected = False
        self._was_connected: bool | None = None  # log state changes only

    async def start(self):
        self._loop = asyncio.get_running_loop()
        self._task = self._loop.create_task(self._run())
        self._task.add_done_callback(self._on_task_done)

    @staticmethod
    def _on_task_done(task):
        """The run task should only ever end via close()'s cancel; anything
        else is a bug we want LOUD, not a silently dead commentary feed."""
        if task.cancelled():
            return
        exc = task.exception()
        print(f"  caster-bridge: run task died unexpectedly: {exc!r}")

    def send(self, text: str, **extra):
        """Queue an event from any thread; drops oldest when backed up.
        `extra` fields merge into the input:text data payload — AIRI reads
        only data.text and ignores the rest, so structured side-channels
        (director beats, HUD numbers for the caster) ride along without
        breaking real-AIRI compatibility."""
        if self._loop is None or self._loop.is_closed():
            return
        self._loop.call_soon_threadsafe(self._enqueue, (text, extra))

    def _enqueue(self, item: tuple):
        while True:
            try:
                self._queue.put_nowait(item)
                return
            except asyncio.QueueFull:
                try:
                    self._queue.get_nowait()  # stale commentary: drop oldest
                except asyncio.QueueEmpty:
                    return

    async def close(self):
        if self._task is not None:
            self._task.cancel()
            try:
                await self._task
            except asyncio.CancelledError:
                pass
            self._task = None

    async def _run(self):
        backoff = 2.0
        pending: tuple | None = None  # event to (re)send; survives reconnects
        while True:
            try:
                async with websockets.connect(self._url) as ws:
                    await self._handshake(ws)
                    self._set_connected(True)
                    backoff = 2.0
                    drain = asyncio.get_running_loop().create_task(
                        self._drain(ws))
                    try:
                        while True:
                            if pending is None:
                                pending = await self._next_event(drain)
                            text, extra = pending
                            await ws.send(json.dumps({
                                "type": "input:text",
                                "data": {"text": text, **(extra or {})},
                            }))
                            pending = None
                    finally:
                        drain.cancel()
            except asyncio.CancelledError:
                raise
            except BaseException as e:
                # BaseException on purpose: a silently swallowed task death
                # here means a dead commentary feed for the whole match
                self._set_connected(False, e)
                await asyncio.sleep(backoff)
                backoff = min(backoff * 2, 60.0)

    async def _next_event(self, drain: asyncio.Task) -> tuple:
        """Wait for the next queued event, but also wake if the server
        closes the connection while we're idle (the drain task ends) —
        otherwise a dead socket goes unnoticed until the next beat."""
        getter = asyncio.ensure_future(self._queue.get())
        done, _ = await asyncio.wait({getter, drain},
                                     return_when=asyncio.FIRST_COMPLETED)
        if getter in done:
            return getter.result()
        getter.cancel()
        raise ConnectionError("server closed the connection while idle")

    async def _handshake(self, ws):
        await ws.send(json.dumps({
            "type": "module:authenticate",
            "data": {"token": _load_token()},
        }))
        async with asyncio.timeout(10):
            while True:
                msg = _unwrap(await ws.recv())
                if msg.get("type") == "module:authenticated":
                    if not msg.get("data", {}).get("authenticated"):
                        raise RuntimeError("AIRI rejected the auth token")
                    break
        # No module:announce: a plain authenticated peer already gets
        # input:text ingested (soak-verified: chat events pushed back,
        # generations fired) and announcing with possibleEvents=[] buys
        # nothing. NOTE the server drops peers with no app-level frame for
        # 60s (websockets protocol pings do NOT refresh its heartbeat) —
        # quiet stretches between beats/games rely on the reconnect loop.

    async def _drain(self, ws):
        """Consume inbound frames (registry syncs etc.) so the connection's
        read queue never applies backpressure."""
        async for _ in ws:
            pass

    def _set_connected(self, up: bool, err: Exception | None = None):
        self._connected = up
        if up != self._was_connected:
            self._was_connected = up
            if up:
                print(f"  caster-bridge: connected to {self._url}")
            else:
                print(f"  caster-bridge: not connected ({err!r}); "
                      "retrying in the background")


async def watch(url: str = DEFAULT_URL):
    """Commentary tap: print each battle beat and the character's reply.

    Read-only peers send no app frames, and the server drops anything
    silent for 60s — so we re-send authenticate every 30s as a keepalive
    and reconnect on any drop. output:gen-ai:chat:complete carries both
    the triggering input (data.text) and the reply (data.message.content);
    the :message variant is skipped to avoid duplicates.
    """
    import time
    while True:
        try:
            async with websockets.connect(url) as ws:
                await ws.send(json.dumps({
                    "type": "module:authenticate",
                    "data": {"token": _load_token()},
                }))
                print("-- tap connected, waiting for commentary --",
                      flush=True)
                last_keepalive = time.monotonic()
                while True:
                    try:
                        raw = await asyncio.wait_for(ws.recv(), timeout=5)
                    except asyncio.TimeoutError:
                        raw = None
                    if time.monotonic() - last_keepalive > 30:
                        await ws.send(json.dumps({
                            "type": "module:authenticate",
                            "data": {"token": _load_token()},
                        }))
                        last_keepalive = time.monotonic()
                    if raw is None:
                        continue
                    msg = _unwrap(raw)
                    if msg.get("type") != "output:gen-ai:chat:complete":
                        continue
                    data = msg.get("data", {})
                    beat = (data.get("text") or "").strip()
                    message = data.get("message") or {}
                    cat = message.get("categorization") or {}
                    # prefer AIRI's extracted 'speech' (drops reasoning) so
                    # this view matches what the relay speaks into the room
                    reply = (cat.get("speech")
                             or message.get("content") or "").strip()
                    stamp = time.strftime("%H:%M:%S")
                    # Label with the speaker the caster actually chose. This
                    # used to be hardcoded "Prism:", so every FRACTURE line in
                    # a transcript was attributed to PRISM — which makes the
                    # transcript useless for the one job it has, catching
                    # mis-attribution and off-contract lines per persona.
                    who = (data.get("persona") or "?").strip().title()
                    if beat:
                        print(f"\n[{stamp}] {beat}", flush=True)
                    if reply:
                        print(f"    {who}: {reply}", flush=True)
        except (KeyboardInterrupt, asyncio.CancelledError):
            raise
        except Exception as e:
            print(f"-- tap disconnected ({e!r}); retrying in 3s --",
                  flush=True)
            await asyncio.sleep(3)


async def _main():
    argv = sys.argv[1:]
    url = DEFAULT_URL
    if "--url" in argv:
        i = argv.index("--url")
        url = argv[i + 1]
        del argv[i:i + 2]
    if "--watch" in argv:
        await watch(url)
        return
    text = argv[0] if argv else "Mic check from the bridge."
    bridge = CasterBridge(url=url)
    await bridge.start()
    bridge.send(text)
    # give the run task time to connect and flush the one event
    for _ in range(50):
        await asyncio.sleep(0.2)
        if bridge._connected and bridge._queue.empty():
            break
    await asyncio.sleep(0.5)  # let the frame actually go out
    await bridge.close()
    print("sent" if bridge._queue.empty() else "NOT sent (AIRI unreachable?)")


if __name__ == "__main__":
    try:
        asyncio.run(_main())
    except KeyboardInterrupt:
        pass
